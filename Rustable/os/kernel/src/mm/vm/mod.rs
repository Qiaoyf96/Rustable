...
pub extern "C" fn vm_init() {
    ...
    kprintln!("boot map segment finished");
...
fn boot_map_segment(pgdir: *mut usize, _va: usize, _size: usize, _pa: usize, perm: usize) {
    ...
    loop {
        ...
        kprintln!("get pte: {:x}", ptep as usize);
...
fn get_pte(pgdir_addr: *mut usize, va: usize, create: bool) -> Result<*mut usize, AllocErr> {
    ...
    let mut pgtable1 = PTE_ADDR(unsafe{ *pgtable0_entry_ptr }) + PT1X(va);
    ...
    let mut pgtable2 = PTE_ADDR(unsafe{ *pgtable1_entry_ptr }) + PT2X(va);
    ...
    let mut pgtable3 = PTE_ADDR(unsafe{ *pgtable2_entry_ptr }) + PT3X(va);
